# Very Simple NetCat Wrapper
class NetCatWrap < Core::CoreShell
	def initialize
		#Basic Info:
		module_name='NetCatWrap'
		module_info={
			'Name'        => 'NetCat Wrapper Module',
			'Version'     => 'v0.01b',
			'Description' => "This is a Simple Wrapper Script for the NetCat Binary (nc). Supports 'Listen' or 'Call' Modes for connecting with bind or reverse shells",
			'Author'      => 'Hood3dRob1n'
		}
	...
	def netcatwrap
		@bin=`which nc`.chomp #Find out the path to NCAT
		check=$?.to_s.split(' ')[3] #Check if we were able to find the path in step above or not (0=true, 1=false)
		if not check.to_i == 0
			puts "[".light_red + "X".white + "] NetCat Does Not appear to be installed on the underlying OS, can't use this plugin without it".light_red + "!".white
			puts
		else
